...
def adjust_learning_rate(optimizer, epoch, base_lr, warmupepochs, lrepochs):
    warm = warmupepochs.split(':')
    splits = lrepochs.split(':')

    upscale_epochs = [int(eid_str) for eid_str in warm[0].split(',')]
    upscale_rate = [float(eid_str) for eid_str in warm[1].split(',')]
    assert len(upscale_rate) == len(upscale_epochs)
    ...
    assert len(downscale_epochs) == len(downscale_rate)

    print("upscale epochs: {}, upscale rate: {}".format(upscale_epochs, upscale_rate))
    print("downscale epochs: {}, downscale rate: {}".format(downscale_epochs, downscale_rate))

    lr = base_lr
    for i, (eid, r) in enumerate(zip(upscale_epochs, upscale_rate)):
        if epoch >= eid:
            lr *= r
        else:
            break

    for i, (eid, r) in enumerate(zip(downscale_epochs, downscale_rate)):
        if epoch >= eid:
            lr /= r
        else:
            break
