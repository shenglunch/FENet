...
def disp_error_image_func_t(D_est_tensor, D_gt_tensor, t, abs_thres=3., rel_thres=0.05, dilate_radius=1):
    D_gt_np = D_gt_tensor.detach().cpu().numpy()
    D_est_np = D_est_tensor.detach().cpu().numpy()
    B, H, W = D_gt_np.shape
    # valid mask
    ...
    error = np.abs(D_gt_np - D_est_np)
    error > t
    print('error mask',t,'=',np.sum(error))
    error_image[np.logical_not(error)] = 255.
...
if __name__ == '__main__':
    ...
    cols = error_colormap
    error_image = np.zeros((1, 20,20*cols.shape[0], 3))
    for i in range(cols.shape[0]):
        distance = 20
        error_image[:, :20, i * distance:(i + 1) * distance, :] = cols[i, 2:]

    error_image=torch.from_numpy(np.ascontiguousarray(error_image.transpose([0, 3, 1, 2])))
    torchvision.utils.save_image(error_image, 'err1.png')

    import matplotlib.pyplot as plt
    import matplotlib 
    matplotlib.use('pdf')
    x = np.linspace(0,2,255)
    x = x[None]
    # x = np.sum(x.squeeze(0), axis=0)

    plt.imshow(x) 
    plt.axis('off')
    plt.savefig('err2.png', bbox_inches = 'tight', pad_inches = 0)